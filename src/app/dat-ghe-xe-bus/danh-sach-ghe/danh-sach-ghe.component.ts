...
export class DanhSachGheComponent implements OnInit {
  ...
  constructor() {}
  ngOnInit() {}
  @ViewChildren(GheItemComponent) listGheItem: QueryList<GheItemComponent>
  outputSeatHandler(seat: ISeat): void {
    let index = this.bookingArr.findIndex(ghe => ghe.SoGhe === seat.SoGhe)
    if (index === -1) {
  ...
  cancelSeat(soGhe:number): void {
    let index = this.bookingArr.findIndex(ghe => ghe.SoGhe === soGhe)
    if (index === -1) {
